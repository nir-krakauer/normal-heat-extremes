...
## -*- texinfo -*-
## @deftypefn  {[@var{paramhat}, @var{FIM}, @var{warnflag}] =} gev_fit_grad_regularized  (@var{data})
## @deftypefnx {[@var{paramhat}, @var{FIM}, @var{warnflag}] =} gev_fit_grad_regularized  (@var{data}, @var{paramguess}, @var{l})
##
...
function [paramhat, FIM, warnflag] = gev_fit_grad_regularized (data, paramguess, l)
  ...
  if (! isvector (data))
    error ("gev_fit_grad_regularized: DATA must be a vector");
  endif
...
## Compute initial parameters if not parsed as an input argument
  ...
  fhandle = @(paramguess) nll_and_grad (paramguess, data, l);
  
  [paramhat, ~, exitflag, output] = fminunc (fhandle, paramguess, options);
  #paramguess
  #[NLL, GRAD] = nll_and_grad (paramguess, data, l)
  #[NLL, GRAD] = nll_and_grad (paramhat, data, l)
  #exitflag, output
  #paramhat
  ...
  if (exitflag == 0)
    if (output.funcCount >= output.iterations)
      warning ("gev_fit_grad_regularized: maximum number of evaluations reached");
    else
      warning ("gev_fit_grad_regularized: reached iteration limit");
    endif
  elseif (exitflag == -1)
    error ("gev_fit_grad_regularized: No solution");
  endif
  ## Return a row vector for Matlab compatibility
  paramhat = paramhat(:)';
  ## Check for second output argument
  if (nargout > 1)
  	[~, GRAD, FIM] = gev_like (paramhat, data);
  	FIM(1, 1) += 2*l;
  	GRAD(1) += 2 * l * paramhat(1);
  	if max(abs(GRAD)) > 1E-3
  	  warning ("gev_fit_grad_regularized:gradient", "gev_fit_grad_regularized: gradient of cost function is not close to zero, solution may not have been reached")
  	  warnflag = true;
  	  disp(GRAD)
    endif
  endif
...
function [NLL, GRAD] = nll_and_grad (parms, data, l)
  [NLL, k_terms] = nll (parms, data);
...
function [nlogL, k_terms] = nll (parms, data)
  k = parms(1);
  log_sigma = parms(2);
  sigma = exp (log_sigma);
  mu = parms(3);
